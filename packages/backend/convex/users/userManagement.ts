...
export const deleteUserAction = internalAction({
    args: { clerkUserId: v.string() },
    handler: async (ctx, args) => {
        const dbUser = await ctx.runQuery(internal.users.userManagement.findDbUserByClerkUserIdQuery, { clerkUserId: args.clerkUserId });
        if (!dbUser) {
            throw new ConvexError('invalid user')
        }
        await ctx.runMutation(internal.users.userManagement.deleteDbUserMutation, { userId: dbUser._id });
    }
});
